...
mod tests {
    ...
    fn diagnostic_code_serialize_as_string() {
        let json = serde_json::to_string(&APL_AI_EVAL_BRIDGE_ASPECT_FAMILY_MISMATCH)
            .expect("serialization must succeed");
        assert_eq!(json, r#""apl-ai-eval-bridge-aspect-family-mismatch""#);

        let json2 = serde_json::to_string(&APL_VALID).expect("serialization must succeed");
        assert_eq!(json2, r#""apl-valid""#);
    }

    #[test]
    fn diagnostic_code_is_ai_eval() {
        assert!(APL_AI_EVAL_PREDICATE_DISALLOWED.is_ai_eval());
        assert!(APL_AI_EVAL_BRIDGE_ASPECT_FAMILY_MISMATCH.is_ai_eval());
        assert!(APL_AI_EVAL_BENCHMARK_ID_MISMATCH.is_ai_eval());
    ...
    fn diagnostic_code_is_pairwise() {
        ...
        assert!(!APL_AI_EVAL_PREDICATE_DISALLOWED.is_pairwise());
    }
